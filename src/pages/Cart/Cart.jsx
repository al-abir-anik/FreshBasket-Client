...
const Cart = () => {
  ...
  const handleQuantity = async (id, currentQty, type) => {
    ...
    if (res.data.modifiedCount > 0) {
      const updated = await fetch(`http://localhost:3000/user-cartlist`);
    ...
    <div className="w-11/12 sm:w-10/12 mx-auto py-6 lg:py-16 flex flex-col lg:flex-row justify-around">
      <div className="flex-1 max-w-4xl">
        <h1 className="text-2xl md:text-3xl font-medium mb-10">
          Shopping Cart{" "}
          <span className="text-sm text-primary pl-3">3 Items</span>
        </h1>
        ...
        {cartlist.map((product, index) => (
          ...
          >
            ...
            {/* action column */}
            <button className="mx-auto p-1.5 border border-red-300 hover:bg-red-50 rounded-full cursor-pointer ">
